...
fish2.spp.with.mito.coding <- fish2.with.mito.coding %>%
  group_by(Species) %>% 
...
fish2.fam.with.mito.coding <- fish2.with.mito.coding %>% 
  group_by(family) %>% 
  summarise(MITO = any(!is.na(COI.acc) & !is.na(CYTB.acc) & COI.acc != "" &  
                         CYTB.acc != "" & COI.cov > 0 &  CYTB.cov !=0)) %>% 
  filter(MITO) %>% 
...
fish2.fam.with.mito.coding.tree <- keep.tip(fishtree, 
                                         fishgenes_genera_accessions %>% filter(TaxonRemoved == "In Tree", 
                                                                                family %in% fish2.fam.with.mito.coding) %>% 
                                           mutate(our_name = str_replace_all(Species, " ", "_")) %>% group_by(family) %>% 
                                           summarise(our_name = first(our_name)) %>% 
                                           pull(our_name))

fish2.fam.with.mito.coding.tree$tip.label <- sapply(fish2.fam.with.mito.coding.tree$tip.label, 
...
fish2.fam.with.mito.coding.pairs <- get.cherries(fish2.fam.with.mito.coding.tree)

## Extract all members of sister pairs with available sequence

fish2.fam.with.mito.coding.pairclades.seq <- sapply(
  fish2.fam.with.mito.coding.pairs, 
...
fish2.fam.with.mito.coding.pairclades.all <- sapply(
  fish2.fam.with.mito.coding.pairs, 
...
fish2.fam.with.mito.coding.pairclades.sampled <- sapply(
  fish2.fam.with.mito.coding.pairclades.seq, 
  phylo.sample.even, 
  simplify = F
)
...
fish2.fam.with.mito.coding.pairtree <- keep.tip(fishtree, 
                                             sapply(fish2.fam.with.mito.coding.pairclades.sampled, 
...
fish2.fam.with.mito.coding.pairtree$tip.label <- 1:Ntip(fish2.fam.with.mito.coding.pairtree)

## Make up final table of pair, accession and quartet assignments

fish2.fam.with.mito.coding.xtets <- find.xtets.sisterpairs(fish2.fam.with.mito.coding.pairtree)

fish2.fam.with.mito.coding.accspairs.t <- bind_rows(
  sapply(1:length(fish2.fam.with.mito.coding.pairclades.sampled),  
         function (x) fishgenes_genera_accessions %>% 
           filter(Species %in% str_replace_all(
             fish2.fam.with.mito.coding.pairclades.sampled[[x]]$tip.label, "_", " ")
           ) %>% 
...
halves <- sapply(fish2.fam.with.mito.coding.pairclades.sampled, 
...
fish2.fam.with.mito.coding.accspairs.t2 <- fish2.fam.with.mito.coding.accspairs.t %>% 
  left_join(clade.names, by="Species")

fish2.fam.with.mito.coding.accspairs.t2$quartet_str <- sapply(
  fish2.fam.with.mito.coding.accspairs.t$pair_no, 
  function (x) paste(which(sapply(fish2.fam.with.mito.coding.xtets, 
                                  function (y) x %in% y)
  ), collapse=" ")
)
