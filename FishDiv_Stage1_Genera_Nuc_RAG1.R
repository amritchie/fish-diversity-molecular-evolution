...
fish2.genera.with.nuc.all <- fish2.with.nuc.all %>% 
  group_by(genus) %>% 
  summarise(NUC = any(!is.na(RAG1.acc) & RAG1.acc != "" & RAG1.cov != 0)) %>% 
  filter(NUC) %>% 
  pull(genus)
...
fish2.genera.with.nuc.all.tree$tip.label <- sapply(fish2.genera.with.nuc.all.tree$tip.label, 
...
fish2.genera.with.nuc.all.pairs <- get.cherries(fish2.genera.with.nuc.all.tree)

## Extract all members of sister pairs with available sequence

fish2.genera.with.nuc.all.pairclades.seq <- sapply(
  fish2.genera.with.nuc.all.pairs, 
...
fish2.genera.with.nuc.all.pairtree <- keep.tip(fishtree, 
                                             sapply(fish2.genera.with.nuc.all.pairclades.sampled, 
...
fish2.genera.with.nuc.all.pairtree$tip.label <- 1:Ntip(fish2.genera.with.nuc.all.pairtree)
...
halves <- sapply(fish2.genera.with.nuc.all.pairclades.sampled, 
