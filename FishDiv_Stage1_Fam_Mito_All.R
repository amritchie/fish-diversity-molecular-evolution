...
fish2.fam.with.mito.all <- fish2.with.mito.all %>% 
  group_by(family) %>% 
  summarise(MITO = any(!is.na(X12s.acc) & !is.na(X16s.acc) & !is.na(COI.acc) & !is.na(CYTB.acc) & 
                         X12s.acc != "" & X16s.acc != "" &  COI.acc != "" &  CYTB.acc != "" &  
                         X12s.cov > 0 &  X16s.cov > 0 &  COI.cov > 0 &  CYTB.cov !=0)) %>% 
...
fish2.fam.with.mito.all.tree <- keep.tip(fishtree, 
                                     fishgenes_genera_accessions %>% filter(TaxonRemoved == "In Tree", 
                                                                            family %in% fish2.fam.with.mito.all) %>% 
                                       mutate(our_name = str_replace_all(Species, " ", "_")) %>% group_by(family) %>% 
                                       summarise(our_name = first(our_name)) %>% 
                                       pull(our_name))

fish2.fam.with.mito.all.tree$tip.label <- sapply(fish2.fam.with.mito.all.tree$tip.label, 
...
fish2.fam.with.mito.all.pairclades.all <- sapply(
  fish2.fam.with.mito.all.pairs, 
...
fish2.fam.with.mito.all.pairclades.sampled <- sapply(
  fish2.fam.with.mito.all.pairclades.seq, 
  phylo.sample.even, 
  simplify = F
  )
...
fish2.fam.with.mito.all.pairtree <- keep.tip(fishtree, 
                                             sapply(fish2.fam.with.mito.all.pairclades.sampled, 
                                                    function (x) x$tip.label[1]))
                                             
fish2.fam.with.mito.all.pairtree$tip.label <- 1:Ntip(fish2.fam.with.mito.all.pairtree)

## Initial arrangement of sister pairs into quartets as outgroup (actual quartets later adjusted manually)

fish2.fam.with.mito.all.xtets <- find.xtets.sisterpairs(fish2.fam.with.mito.all.pairtree)

## Make up final table of pair, accession and quartet assignments

fish2.fam.with.mito.all.accspairs.t <- bind_rows(
  sapply(1:length(fish2.fam.with.mito.all.pairclades.sampled),  
         function (x) fishgenes_genera_accessions %>% 
           filter(Species %in% str_replace_all(
             fish2.fam.with.mito.all.pairclades.sampled[[x]]$tip.label, "_", " ")
             ) %>% 
...
halves <- sapply(fish2.fam.with.mito.all.pairclades.sampled, 
...
fish2.fam.with.mito.all.accspairs.t2 <- fish2.fam.with.mito.all.accspairs.t %>% 
  left_join(clade.names, by="Species")

fish2.fam.with.mito.all.accspairs.t2$quartet_str <- sapply(
  fish2.fam.with.mito.all.accspairs.t$pair_no, 
  function (x) paste(which(sapply(fish2.fam.with.mito.all.xtets, 
                            function (y) x %in% y)
                     ), collapse=" ")
  )
...
fish2.fam.with.mito.all.12S.seqs <- read.GenBank(fish2.fam.with.mito.all.accspairs.t$X12s.acc)
fish2.fam.with.mito.all.16S.seqs <- read.GenBank(fish2.fam.with.mito.all.accspairs.t$X16s.acc)
fish2.fam.with.mito.all.COI.seqs <- read.GenBank(fish2.fam.with.mito.all.accspairs.t$COI.acc)
