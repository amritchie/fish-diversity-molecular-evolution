...
bigfish.tmp.analyse.data <- function (qtable, lifetable, est.trees, lifetrees, taxon){
  ...
  for (sister in clades)
  {
    ...
    quartets <- ((qtable %>% filter(clade_name == sister))[1,] %>%
      pull(quartets))[[1]]
...
## END COMMENT OUT ##
  ...
  print(dim(out.t)[1])
  #ggplot(fish2.genera.with.mito.life.out %>% melt(id.vars=c("Family_first", "Genus_first", "Sister_first", "Family_last", "Genus_last", "Sister_last", "Pair")) %>% mutate(vartype = sapply(str_split(variable, "_"), function (x) x[[1]][1]), varfunc = sapply(variable, function (x) str_split(x, "_")[[1]][length(str_split(x, "_")[[1]])])) %>% filter(!(varfunc %in% c("diff", "std")), !(vartype %in% c("Age", "swap", "time.std", "rate.std"))) %>% group_by(Pair, vartype) %>%  summarise(diff = abs(first(value) - last(value)),  m = mean(c(first(value), last(value))))) + geom_point(aes(x = diff, y = m)) + facet_wrap(facets = "vartype") + labs(x = "Absolute branch length contrast/Root(pair age)", y = "Mean branch length") + theme(panel.background = element_blank(), axis.line = element_line(colour = "black", size = 0.5), text = element_text(family="serif"))
  
  return(out.t)

}

####tf <- bigfish.tmp.analyse.data(fish2.fam.with.mito.accspairs.t2, fishgenes_genera_accessions, fish2_fam_with_mito_baseml_trees, fish2.fam.with.mito.pairclades.all, "Family")
